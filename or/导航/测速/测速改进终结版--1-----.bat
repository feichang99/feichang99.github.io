...
set "FINAL_OUT=YouTube优选网速排名.txt"

echo =======================================================
echo    Cloudflare YouTube 专用流媒体多线程 IP 优选测速工具
echo =======================================================
echo.
echo  * 测速说明: 每个IP将执行多连接下载测试，评估视频串流能力
echo  * 超时控制: 优化防卡死机制，精准匹配 YouTube 节点
echo -------------------------------------------------------
...
if exist "%TEMP_OUT%" (
    echo.
    echo -------------------------------------------------------
    echo            ★ YouTube 优质 IP 排名（按流媒体表现排序） ★
    echo -------------------------------------------------------
    echo  串流网速        ^| 地址 ^& 备注
    echo -------------------------------------------------------
    ...
    :: 写入最终排名文件的头部说明
    echo ========================================================== > "%FINAL_OUT%"
    echo      YouTube 专用优选网速排名（第一行最适合看视频） >> "%FINAL_OUT%"
    echo ========================================================== >> "%FINAL_OUT%"
    echo. >> "%FINAL_OUT%"
    ...
    echo.
    echo -------------------------------------------------------
    echo 提示：优化结果已保存到同目录下的 [%FINAL_OUT%] 中！
) else (
    echo.
...
:: 3. 核心调用：使用 curl 模拟多并发短数据段抓取（更贴近视频流分片加载）
set "speed_bytes="
for /f "delims=" %%i in ('curl -o NUL -s -w "%%{speed_download}" --connect-timeout 2 --max-time 5 --connect-to speed.cloudflare.com:443:%addr% "https://speed.cloudflare.com/__down?bytes=2000000" 2^>nul') do (
    set "speed_bytes=%%i"
)
...
echo  NO.%f_no% ^| 流媒体网速: %f_speed% ^| IP地址: %f_addr% ^| 地区: %f_remark% >> "%FINAL_OUT%"
goto :eof
